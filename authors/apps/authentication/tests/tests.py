...
class Authentication(APITestCase):
    ...
    def test_for_new_user(self):
        ...
        data = {"user": {"username": "akram",
                         "email": "akram@gmail.com", "password": "akrammukasa"}}
    ...
    def test_for_missing_username(self):
        ...
        """
        data = {"user": { "username":"", "email": "akram@gmail.com", "password":"akrammukasa"}}
        response = self.client.post('/api/users/' ,data, format='json')
    ...
    def test_for_missing_email(self):
        ...
        """
        data = {"user": { "username":"mukasa", "email": "", "password":"akrammukasa"}}
        response = self.client.post('/api/users/' ,data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("This field may not be blank.", str(response.data))
    ...
    def test_login(self):
        ...
        response = self.client.post('/api/users/', data, format='json')
        # url = reverse('login')
        data = {"user": {"email": "akram@gmail.com", "password": "akrammukasa"}}
